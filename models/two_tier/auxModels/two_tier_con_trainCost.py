...
#loop over all para files
#paraPath = 'results_2t/models-two_tier-two_tier_con.py-expCON_T4T_UNCONINIT_FULLLAB-n_frames40-frame_size20-emb_size256-skip_connF-dim1024-n_rnn3-rnn_typeGRU-q_levels256-q_typelinear-batch_size20-weight_normT-learn_h0T-which_setSPEECH-lr0.001/params/'

paraPath = 'results_2t/models-two_tier-two_tier_con.py-expCON_T4T_ZLINIT_FULLLAB-n_frames40-frame_size20-emb_size256-skip_connF-dim1024-n_rnn3-rnn_typeGRU-q_levels256-q_typelinear-batch_size20-weight_normT-learn_h0T-which_setSPEECH-lr0.001/params/'
...
try:
    trainCost_list = list(numpy.load(paraPath[:-7]+'trainCost.npy'))
    trainIter_list = list(numpy.load(paraPath[:-7]+'trainIter.npy'))
    print('found existing npy, starting from there')
except IOError:
    trainCost_list = []
    trainIter_list = []
    print('starting fresh')
    
for paraFile in os.listdir(paraPath):
    if paraFile.find('_i')!=-1:
        start = paraFile.find('_i') + 2
        end = paraFile.find('_t')
        i = float(paraFile[start:end])
        print('iteration '+str(i)+': '),
        if i in trainIter_list:
            print('already there')
        else:
            print('computing...')
            trainIter_list.append(i)

            #1 load para
            print('---loading para pkl---')
            lib.load_params(paraPath + paraFile)
            print('---loading complete---')

            #2 compute training cost, add to list
            train_cost, train_time = monitor(train_feeder)
            trainCost_list.append(train_cost)
            print('time spent in hour: '+str(train_time/3600.))

            #3 make numpy array, save as npy
            #numpy.save(paraPath[:-7]+'trainCost_unSort.npy',numpy.array(trainCost_list))
            #numpy.save(paraPath[:-7]+'trainIter_unSort.npy',numpy.array(trainIter_list))

            trainIter_list, trainCost_list = (list(t) for t in zip(*sorted(zip(trainIter_list, trainCost_list))))
            numpy.save(paraPath[:-7]+'trainCost.npy',numpy.array(trainCost_list))
            numpy.save(paraPath[:-7]+'trainIter.npy',numpy.array(trainIter_list))
