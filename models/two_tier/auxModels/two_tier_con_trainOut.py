...
trainOut = T.nnet.softmax(sample_level_outputs)
trainRef = target_sequences.flatten()
...
while True:
    ...
    start_time = time()
    cost, h0, trainOut, trainRef = train_fn(seqs, seqs_lab, h0, reset, mask)
    ...
    if total_iters % 500 == 0:
        print total_iters,
        numpy.save('bkup_data/trainOut.npy',trainOut)
        numpy.save('bkup_data/trainRef.npy',trainRef)
        #-------------------------------------------
        pdb.set_trace() #debug
        #-------------------------------------------
    total_iters += 1
