...
assert len(FRAME_SIZE_LIST) == len(RNN_DEPTH_LIST),'FRAME_SIZE_LIST & RNN_DEPTH_LIST should have the same length'
...
RNN_DEPTH_LIST.append(0)
UP_RATE_LIST = []
for c,n in zip(FRAME_SIZE_LIST[:-1],FRAME_SIZE_LIST[1:]):
    assert c % n == 0,'current frame_size should be divisible by next frame_size'
    UP_RATE_LIST.append(c/n)
###
...
def frame_level_rnn(input_sequences, other_input, h0, reset, idx=1):
    ...
    n_rnn = RNN_DEPTH_LIST[idx]
    up_rate = UP_RATE_LIST[idx]
...
NB_INTER_TIERS = len(FRAME_SIZE_LIST)-2 #exclude top and bottom (sample-level) tier
INTER_TIER_IDX_LIST = range(1,1+NB_INTER_TIERS)
for idx in INTER_TIER_IDX_LIST:
    h0_list.append(T.tensor3('h0_{}'.format(idx)))
...
if args.debug:
    ...
    n_rnn = RNN_DEPTH_LIST[0]
    h0_list[0].tag.test_value = numpy.zeros((BATCH_SIZE, n_rnn, H0_MULT*DIM), dtype='float32')
    # big_h0.tag.test_value = numpy.zeros((BATCH_SIZE, N_RNN, H0_MULT*BIG_DIM), dtype='float32')
    for idx in INTER_TIER_IDX_LIST:
        n_rnn = RNN_DEPTH_LIST[idx]
        h0_list[idx].tag.test_value = numpy.zeros((BATCH_SIZE, n_rnn, H0_MULT*DIM), dtype='float32')
...
input_sequences_list.append(big_input_sequences)
for idx in INTER_TIER_IDX_LIST:
    frame_size = FRAME_SIZE_LIST[idx]
    input_sequences_list.append(sequences[:, BIG_FRAME_SIZE-frame_size:-frame_size])
...
new_h0_list.append(new_big_h0)
for idx in INTER_TIER_IDX_LIST:
    tmp_inp = input_sequences_list[idx]
    tmp_con = frame_level_outputs_list[idx-1]
    tmp_h0 = h0_list[idx]
    tmp_out,tmp_new_h0 = frame_level_rnn(tmp_inp, tmp_con, tmp_h0, reset,idx=idx)
    frame_level_outputs_list.append(tmp_out)
    new_h0_list.append(tmp_new_h0)


frame_size = FRAME_SIZE_LIST[-2]
...
sample_level_outputs = sample_level_predictor(
    frame_level_outputs_list[-1].reshape((BATCH_SIZE * SEQ_LEN, DIM)),
    prev_samples
)
...
train_fn = theano.function(
    mk_one_list([sequences, h0_list, reset, mask]),
    mk_one_list([cost, new_h0_list]),
    updates=updates,
    on_unused_input='warn'
)
# train_fn_asup = theano.function(
#     [sequences, h0_list, reset, mask],
#     [cost, new_h0_list],
#     updates=updates,
#     on_unused_input='warn'
# )
...
#preparation
gen_big_frame_level_outputs = T.matrix('gen_big_frame_level_outputs')
gen_frame_level_outputs_list = []
gen_frame_level_outputs_list.append(gen_big_frame_level_outputs)
for idx in INTER_TIER_IDX_LIST:
    gen_frame_level_outputs_list.append(T.matrix('gen_frame_level_outputs_{}'.format(idx)))
...
gen_fn_list = []
gen_fn_list.append(big_frame_level_generate_fn)
# Sampling at frame level
for idx in INTER_TIER_IDX_LIST:
    tmp_fn = theano.function(
        [sequences, gen_frame_level_outputs_list[idx-1], h0_list[idx], reset],
        frame_level_rnn(sequences, gen_frame_level_outputs_list[idx-1].dimshuffle(0,'x',1), h0_list[idx], reset,idx=idx),
        on_unused_input='warn'
    )
    gen_fn_list.append(tmp_fn)
# Sampling at audio sample level
sample_level_generate_fn = theano.function(
    [gen_frame_level_outputs_list[-1], prev_samples],
    lib.ops.softmax_and_sample(
        sample_level_predictor(
            gen_frame_level_outputs_list[-1],
            prev_samples
        )
    ),
    on_unused_input='warn'
)
gen_fn_list.append(sample_level_generate_fn)
...
fixed_rand_h0_list = []
fixed_rand_h0_list.append(fixed_rand_big_h0)
for idx in INTER_TIER_IDX_LIST:
    fixed_rand_h0 = numpy.random.rand(N_SEQS//2, RNN_DEPTH_LIST[idx], H0_MULT*DIM)
    fixed_rand_h0 -= 0.5
    fixed_rand_h0 = fixed_rand_h0.astype('float32')
    fixed_rand_h0_list.append(fixed_rand_h0)
...
def generate_and_save_samples(tag):
    ...
    for idx in INTER_TIER_IDX_LIST:
        tmp_h0 = numpy.zeros(
                (N_SEQS-fixed_rand_h0_list[idx].shape[0], RNN_DEPTH_LIST[idx], H0_MULT*DIM),
                dtype='float32'
        )
        ...
        for idx in INTER_TIER_IDX_LIST:
            if t % FRAME_SIZE_LIST[idx] == 0:
                frame_level_outputs_list[idx],h0_list[idx]=gen_fn_list[idx](
                    ...
                    frame_level_outputs_list[idx-1][:, (t / FRAME_SIZE_LIST[idx]) % (FRAME_SIZE_LIST[idx-1] / FRAME_SIZE_LIST[idx])],
                    h0_list[idx],
                    numpy.int32(t == BIG_FRAME_SIZE)
                )
        ###QDOU: soft code
